...
function replicateDrugDRCPlotsHTS(dataAfterFit,drugsCellLinesThisFolder,folder)
    ...
    for drug = 1:size(drugs,2)
        ...
        %List all fits
        if isfield(dataAfterFit.fitObj.IndFit,(drugs{drug}))
            allFits = fieldnames(dataAfterFit.fitObj.IndFit.(drugs{drug}));
        end
        ...
        for cellLine = 1:size(cellLines,2)
            ...
            for replicate = 1:size(replicateFitNames,1)
                ...
                fittedHill = dataAfterFit.fitObj.IndFit.(drugs{drug}).(replicateFitNames{replicate});
