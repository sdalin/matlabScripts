...
%across all plates.
%numReps is the number of replicates you are using here.
...
function [fittedStruct,individualFittedStruct] = hillFitv2(bigstructNormed,concentrations,numReps)
...
%to store ANOVA statistics
p = zeros(1,size(plates,1));
tbl = cell(1,size(plates,1));
stats = cell(1,size(plates,1));

for k = 1:size(plates,1)
    numColumns = size(bigstructNormed.(char(plates(k,:))),1)/16;
    fittedStruct.(char(plates(k,:))) = zeros(9,numColumns);
    ...
    for cellLine = 1:size(bigstructNormed.(char(plates(k,:))),1)/16
        ...
        thisCellLine = bigstructNormed.(char(plates(k,:)))(1+(16*(cellLine-1)):(16+(16*(cellLine-1))),1:numReps);
        ...
        if length(thisCellLine) > 3
            ...
            for replicate = 1:numReps
                ...
                if length(currentCellLine) > 3
                    %The fit
                    [individualFittedHill{((cellLine-1)*numReps)+replicate}, individualGof{((cellLine-1)*numReps)+replicate}] = fit(currentConcs,currentCellLine, hill, opts);
                else
                    continue
                end
            end
            ...
            for field = 1:size(stErrVariables,1)
                ...
                currentVariable = zeros(numReps,1);
                for replicate = 1:numReps
                    if ~isempty(individualFittedHill{((cellLine-1)*numReps)+replicate})
                        currentVariable(numReps,1) = individualFittedHill{((cellLine-1)*numReps)+replicate}.(char(stErrVariables(field,:)));
                    else
                        continue
                    end
                end
                individualGof{((cellLine-1)*numReps)+1}.stErr.(char(stErrVariables(field,:))) = std(currentVariable)/sqrt(numReps);
            end
        ...
        for field = 1:size(stErrVariables,1)
            ...
            for replicate = 1:numReps
                if ~isempty(individualFittedHill{((cellLine-1)*numReps)+replicate})
                    individualFittedStruct.(char(plates(k,:)))(field,((cellLine-1)*numReps)+replicate) = individualFittedHill{((cellLine-1)*numReps)+replicate}.(char(stErrVariables(field,:)));
                else
                    continue
                end
                
                if isfield(individualGof{((cellLine-1)*numReps)+replicate},'stErr')
                    individualFittedStruct.(char(plates(k,:)))(field+4,((cellLine-1)*numReps)+replicate) = individualGof{((cellLine-1)*numReps)+replicate}.stErr.(char(stErrVariables(field,:)));
                else
                    continue
                end
            end
        ...
        for replicate = 1:numReps
            if isfield(individualGof{((cellLine-1)*numReps)+replicate},'rsquare')
                individualFittedStruct.(char(plates(k,:)))(9,((cellLine-1)*numReps)+replicate) = individualGof{((cellLine-1)*numReps)+replicate}.rsquare;
            else
                continue
            end
        end
    ...
    %each other? (ANOVA on individual fits only)
    

    %[p(k),tbl{k},stats{k}] = anova1(reshape(individualFittedStruct.(char(plates(k,:)))(3,:),[numReps,numColumns]));
...
cd('/Users/sdalin/Dropbox (MIT)/Biology PhD/2016/Hemann Lab/CR.CS/DRCs on Resistant Cells/Resistant Cells Round 3/matlabOutput')
for k = 1:size(plates,1)
...
%Plot it
%hillPlot160610(fittedHill,concentrations,bigstructNormed,numReps);
