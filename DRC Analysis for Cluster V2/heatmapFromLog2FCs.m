...
function [heatmapMatrixCleaned,cellLines,drugsCleaned] = heatmapFromLog2FCs(dataAfterFit,folder,stars,varargin)
    ...
    foldChangeHeatmap = clustergram(heatmapMatrixCleaned,'RowLabels',cellLines,'ColumnLabels',drugsCleaned,'DisplayRange',log2Scale,'Symmetric','true','Colormap',redbluecmap,'RowLabelsRotate',0,'ImputeFun',@knnimpute);
    ...
    for cellLine = 1:size(foldChangeHeatmap.RowLabels,1)
        for drug = 1:size(foldChangeHeatmap.ColumnLabels,2)
            ...
            [heatmapDrugRow,col] = find(strcmp(drugsCleaned,sprintf('%s',foldChangeHeatmap.ColumnLabels{drug})));
            ...
            if ~isempty(stars{drugRow,cellLineColumn}) && ~isnan(heatmapMatrixCleaned(heatmapCellLineColumn,heatmapDrugRow))
                if nargin > 3
