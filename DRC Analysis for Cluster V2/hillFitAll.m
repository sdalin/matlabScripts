...
function [dataAfterFit,drugsCellLinesThisFolder] = hillFitAll(bigstructNormed,folderName)
            ...
            if ~strcmp(cellLines{cellLine},'Info')
                ...
                for drug = 1:size(drugs,2)
                    ...
                    if sum(thisCellLine > 0.3) < 2
                        ...
                        allKiller{end+1} = sprintf('drug_%s_%s_%s',drugs{drug},cellLines{cellLine},experiments{experiment});

                        %make scatterplot and save to
                        %'rejectedFits' folder
                        plotIndividualCellLines(fittedHill{cellLine},fittedHillNoOutliers{cellLine},currentConcs,thisCellLine,outliers,'not fit',drugs{drug},cellLines{cellLine},experiments{experiment},folderName,skipToNextCellLine);

                    %Check if at least two points are viability less
                    %than 0.7
                    elseif sum(thisCellLine < 0.7) < 2
                        skipToNextCellLine = 1;
                        nonKiller{end+1} = sprintf('drug_%s_%s_%s',drugs{drug},cellLines{cellLine},experiments{experiment});
                    ...
                    elseif length(thisCellLine) > 3
                        ...
                        if gofNoOutliers{cellLine}.rsquare < 0.8
                            ...
                            badFit{end+1} = sprintf('drug_%s_%s_%s',drugs{drug},cellLines{cellLine},experiments{experiment});

                            %make scatterplot and save to
                            %'rejectedFits' folder
                            plotIndividualCellLines(fittedHill{cellLine},fittedHillNoOutliers{cellLine},currentConcs,thisCellLine,outliers,gofNoOutliers{cellLine}.rsquare,drugs{drug},cellLines{cellLine},experiments{experiment},folderName,skipToNextCellLine); 
                        end
                        ...
                        fitObj.IndFit.(sprintf('drug_%s',drugs{drug})).(sprintf('%s_%s',cellLines{cellLine},experiments{experiment})) = fittedHillNoOutliers{cellLine};
                        gofObj.IndFit.(sprintf('drug_%s',drugs{drug})).(sprintf('%s_%s',cellLines{cellLine},experiments{experiment})) = gofNoOutliers{cellLine};

                        %Indicate this drug/cell line was fit
                        dataFit{end+1} = sprintf('drug_%s_%s_%s',drugs{drug},cellLines{cellLine},experiments{experiment});
                        ...
                        skipToNextCellLine = 1;
                        lessThanThreeDataPoints{end+1} = sprintf('drug_%s_%s_%s',drugs{drug},cellLines{cellLine},experiments{experiment});
                    ...
                    if ~skipToNextCellLine
                        rawData.(sprintf('drug_%s',drugs{drug})).(cellLines{cellLine}) = [num2cell(currentConcs),num2cell(thisCellLine)];

                         %Now extract the EC50 of each cell line/drug into the fitParams field of dataAfterFit
                        currentEC50 = fitObj.IndFit.(sprintf('drug_%s',drugs{drug})).(sprintf('%s_%s',cellLines{cellLine},experiments{experiment})).IC50;
                    ...
                    if ~isfield(fitParams,(sprintf('drug_%s',drugs{drug})))
                        fitParams.(sprintf('drug_%s',drugs{drug})).EC50 = cell(0);
                    end
                    
                    [row,column] = find(strcmp(fitParams.(sprintf('drug_%s',drugs{drug})).EC50,sprintf('%s',cellLines{cellLine})));
                    
                    %If that cell line has no data yet, add new column with
                    %cell line name and EC50
                    if isempty(row)
                        cellLineAndEC50 = cell(size(fitParams.(sprintf('drug_%s',drugs{drug})).EC50,1),1);
                        row = 2;
                        cellLineAndEC50(1) = {sprintf(cellLines{cellLine})};
                        cellLineAndEC50(row) = {currentEC50};
                        fitParams.(sprintf('drug_%s',drugs{drug})).EC50(:,size(fitParams.(sprintf('drug_%s',drugs{drug})).EC50,2)+1) = cellLineAndEC50;
                    end
